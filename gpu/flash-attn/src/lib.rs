...
//! probabilities. Backward recomputes those probabilities and fuses dQ, dK,
//! and dV. One block owns one `(batch, head)` and each lane owns one feature
//! across the sequence, making all gradient writes disjoint without atomics.
//!
...
//! - backward launches `(rows / sequence_length) * heads` blocks.
