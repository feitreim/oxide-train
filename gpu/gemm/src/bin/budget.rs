...
//!    Its `no drain` twin prices the epilogue that remains after #83, and its
//!    **`no ping`** twin turns the accumulator ping-pong off with everything
//!    else held — same 512 columns, same six stages, same 74 clusters — so the
//!    `ACC` column of the two arms is what the second accumulator bought.
//!
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for name in [
        "gemm_probe_f32_store",
        "gemm_probe_f32_nodrain",
        "gemm_probe_f32_noping",
    ] {
...
fn phases(
    ...
    let solo = time_gpu_iters(stream, WARMUP, ITERS, || {
        unsafe {
            probe.gemm_probe_f32_noping(
                stream,
                config,
                a_tma.as_ptr(),
                b_tma.as_ptr(),
    ...
    let solo_counters = clocks.to_host_vec(stream)?;
    ...
    let median_of = |ticks: &[u64], slot: usize| -> f64 {
        let mut per_item: Vec<f64> = (0..clusters)
            .map(|cluster| ticks[device::COUNTERS * cluster + slot] as f64)
            .collect();
        per_item.sort_by(f64::total_cmp);
        per_item[clusters / 2]
    };
    let median = |slot: usize| median_of(&counters, slot);
    let solo_median = |slot: usize| median_of(&solo_counters, slot);
    ...
    println!(
        "    no ping (one accumulator slot, everything else held) {solo:.4} ms ({:+.1}% vs \
         probe): ACC {:.0} + MMA {:.0}, DRAIN {:.0}, DONE {:.0}, item {:.0}",
        100.0 * (solo - store) / store,
        solo_median(device::ACC),
        solo_median(device::MMA),
        solo_median(device::DRAIN),
        solo_median(device::DONE),
        solo_median(device::SPAN),
    );
