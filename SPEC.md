...
9. **Scale-up (in progress)** — depth + width toward MFU. Structural half
   ...
   - ✅ **Combined stack** (#43+#44+#45, merged as #49/#50/#52): single
     ...
     **full step 618.1 ms** (from the 748–751 ms pre-stack baselines,
     ...
     levers tracked in #47 (M128-pairing model-shape A/B), #54
     (weight-reuse — now the *only* lever left on the weight-grad spans,
     since #53 showed operand orientation is not one), #55 (float4
     scatter + `zero_dy_bins` fold).
